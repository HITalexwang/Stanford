...
class Bucketer(Configurable):
  ...
  def compute_splits(self, data, plot=True):
    ...
    i = 0
    if self.verbose:
      print('Initial # of tokens in buckets:', str(self.size()))
    while self != old_splits:
      old_splits = list(self)
      self.recenter()
      i += 1
    if self.verbose:
      print('Final # of tokens in buckets:', str(self.size()))
