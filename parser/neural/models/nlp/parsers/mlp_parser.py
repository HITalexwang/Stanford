...
class MLPParser(BaseParser):
  ...
  def score(self, dep_mlp, head_mlp, output=1):
    ...
    # (n x b x b x mlp_size)
    mlp = self.MLP(reps, self.mlp_size)
    # (n x b x b x output)
    #logits = self.linear(reps, output, initializer=tf.random_normal_initializer)
    with tf.variable_scope('Reduce'):
      logits = self.linear(mlp, output, initializer=tf.random_normal_initializer)
  ...
  def parse4graph(self, vocabs, ts_lstm=None, arc_placeholder=None, stacked_cnn=None, moving_params=None):
    ...
    with tf.variable_scope('Arc'):
      # (n x b x 2 x b)
      #arc_logits = self.bilinear(arc_dep_mlp, arc_head_mlp, 2, add_bias2=False)
      #arc_logits = tf.transpose(arc_logits, [0, 1, 3, 2])
      # (n x b x b x 2)
      arc_logits = self.score(arc_dep_mlp, arc_head_mlp, 2)
      # (n x b x b) of 0/1
      arc_targets = self.vocabs['heads'].placeholder
      n_arc_targets = tf.reduce_sum(tf.reduce_sum(arc_targets, axis=2) * int_tokens_to_keep)
      # (n x b x b) of 0/1
      arc_preds = tf.to_int32(tf.argmax(arc_logits, axis=3))
      ...
      # (n x b x 1)
      tokens_to_keep_ = tf.expand_dims(self.tokens_to_keep, axis=2)
      # ()
      arc_loss = tf.losses.sparse_softmax_cross_entropy(arc_targets, arc_logits, tokens_to_keep_)
    ...
    with tf.variable_scope('Rel'):
      ...
      # ()
      #rel_loss = tf.losses.sparse_softmax_cross_entropy(rel_targets, rel_logits, arc_preds)
      rel_loss = 0
